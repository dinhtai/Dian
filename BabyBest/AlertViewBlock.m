...
+ (void)showMessage:(NSString *)message withButton:(NSString *)btnString withBlock:(VoidBlock)aBlock {
    ...
    AlertViewBlock *alertBlock = [[AlertViewBlock alloc] init];
    [alertBlock showMessage: message withButton: btnString withBlock: aBlock];
}
...
+ (void) showConfirmWithTwoButton:(NSString *)msg strLeftButton:(NSString *)leftButton
    ...
    AlertViewBlock *alertBlock = [[AlertViewBlock alloc] init];
    [alertBlock showConfirmWithTwoButton:msg strLeftButton:leftButton srtRightButton:rightButton withBlock:aBlock];
}
...
- (void) showConfirmWithTwoButton:(NSString *)msg strLeftButton:(NSString *)leftButton
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle: @"" message: msg delegate: self cancelButtonTitle: leftButton otherButtonTitles: rightButton, nil];
    [alert show];
    [alert release];
...
- (void)showConfirmRetryMessage:(NSString *)message withTitle:(NSString *)title withBlock:(AlertBlock)aBlock {
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle: title message: message delegate: self cancelButtonTitle: NSLocalizedString(@"RETRY", nil) otherButtonTitles: nil];
    [alert show];
    [alert release];
}
...
- (void)showConfirmGoMessage:(NSString *)message withBlock:(AlertBlock)aBlock {
    ...
    self.confirmBlock = aBlock;
    self.style = Confirm_Yes_No;
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle: @"" message: message delegate: self cancelButtonTitle: NSLocalizedString(@"OK", nil) otherButtonTitles: NSLocalizedString(@"GO", nil), nil];
    [alert show];
    [alert release];
}
...
- (void)showMessage:(NSString *)message withButton:(NSString*)buttonString withBlock:(VoidBlock)aBlock {
    
    self.okBlock = aBlock;
    self.style = Only_OK;
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle: @"" message: message delegate: self cancelButtonTitle: NSLocalizedString(buttonString, nil) otherButtonTitles: nil];
    [alert show];
    [alert release];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    switch (self.style) {
        case Confirm_Yes_No: {
//             Comment đoạn này để chỉ show popup với 1 button Retry
                if(buttonIndex == 0) {
                            //OK
...
// binhdt: chống bấm 2 nút trên alertdialog đồng thời
